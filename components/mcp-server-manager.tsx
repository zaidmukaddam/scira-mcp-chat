...
export const MCPServerManager = ({
  ...
  const addServer = () => {
    ...
    if (
      ...
      (!newServer.command || !newServer.args?.length)
    ) {
      toast.error(
        'Command and at least one argument are required for stdio transport',
      );
      return;
    }
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between mb-2">
                              <div className="flex items-center gap-2">
                                {server.type === 'sse' ? (
                                  <Globe
                                    className={`h-4 w-4 ${isActive ? 'text-primary' : 'text-muted-foreground'} flex-shrink-0`}
                                  />
                                ) : (
                                  <Terminal
                                    className={`h-4 w-4 ${isActive ? 'text-primary' : 'text-muted-foreground'} flex-shrink-0`}
              ...
              <div className="grid gap-1.5">
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid gap-2 grid-cols-2">
                    ...
                    >
                      <Globe
                        className={`h-5 w-5 shrink-0 ${newServer.type === 'sse' ? 'text-primary' : ''}`}
                      />
                      <div>
                    ...
                    >
                      <Terminal
                        className={`h-5 w-5 shrink-0 ${newServer.type === 'stdio' ? 'text-primary' : ''}`}
                      />
                      <div>
